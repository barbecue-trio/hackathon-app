...
function extractMenuNamesFromText(
  text: string
): { name: string; name_jp: string }[] {
...
// 食文化生成のメイン関数
async function generateFoodCultureForDocument(
  documentId: string
): Promise<number> {
  try {
...
// カテゴリー生成のメイン関数
async function generateCategoriesForDocument(
  documentId: string
): Promise<number> {
  try {
    const docRef = db.collection("menu_collections").doc(documentId);
    ...
    console.log(
      `${menuCollection.menus.length}個のメニューのカテゴリーを一括でAI判定中...`
    );

    // 全メニューをまとめてAIに投げてカテゴリー判定
    const categoryResults = await determineCategoriesForAllMenus(
    ...
    for (let i = 0; i < updatedMenus.length; i++) {
      ...
      console.log(
        `カテゴリーID ${categoryResults[i]} を割り当てました: ${updatedMenus[i].name}`
      );
    }
...
async function determineCategoriesForAllMenus(
  menus: MenuItem[]
): Promise<number[]> {
  try {
    const apiKey = geminiApiKey.value();
